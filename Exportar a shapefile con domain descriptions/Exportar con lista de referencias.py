...
ellas, se la exporta a shapefile en una carpeta llamada como el dataset
que se crea dentro de la carpeta de salida y en la misma se genera un 
...
def escribir_archivos_atributos (GDB, outLocation):
	...
	for dataset in datasets:
		...
		carpeta_actual = os.path.join(outLocation, dataset)
		os.mkdir(carpeta_actual)
		...
		for feature in arcpy.ListFeatureClasses():
			...
			arcpy.conversion.FeatureClassToFeatureClass(feature, carpeta_actual, nuevo_nombre)

			# Obtengo los nombres de los campos como figuran en el shapefile
			# recién creado
			lista_nombres_incorrectos = obtener_lista_de_Campos(os.path.join(carpeta_actual, nuevo_nombre))
			...
			# listas de atributos
			archivo_atributos = crear_csv_escritura(os.path.basename(feature)+'_atributos', carpeta_actual)
